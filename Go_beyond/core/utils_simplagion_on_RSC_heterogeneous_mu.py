...
class SimplagionModel():
    def __init__(self, node_neighbors_dict, triangles_list, I_percentage, mu_array=None):
            ...
            if len(self.iAgentSet)<self.N:
                ...
                for recoverAgent in self.iAgentSet:
                    ...
                    if recoverAgent in newIlist:
                        ...
                        node_mu = self.mu_array[recoverAgent]
                        if (random.random() <= node_mu):
...
def run_one_simulation(args):
    ...
    real_k  = 1. * sum([len(v) for v in node_neighbors_dict.values()]) \
    ...
    nodes_list = list(node_neighbors_dict.keys())
    
    degrees = {n: len(list(node_neighbors_dict[n])) for n in nodes_list}
    k_avg   = np.mean(list(degrees.values()))
    
    alpha = 0.5
    raw_mu  = {n: mu / (alpha + degrees[n] / k_avg) for n in nodes_list}
    
    raw_mean = np.mean(list(raw_mu.values()))
    mu_dict  = {n: raw_mu[n] * (mu / raw_mean) for n in nodes_list}
    
    mu_array = {n: mu_dict[n] for n in nodes_list}

    rhos = []

    for beta1 in beta1s:
        mySimplagionModel = SimplagionModel(
            node_neighbors_dict, triangles_list, I_percentage,
            mu_array=mu_array          
        )
        mySimplagionModel.initial_setup(
            fixed_nodes_to_infect=mySimplagionModel.initial_infected_nodes
        )
        results = mySimplagionModel.run(
            t_max, beta1, beta2, mu,   
            print_status=False
        )
...
def find_cut(rhos_array):
    nonzeros_per_col = np.count_nonzero(rhos_array, axis=0)
    valid = np.where(nonzeros_per_col > 1)[0]
...
def parse_results(results, cut, return_std=False):
    ...
    if cut == False:    
        ...
        if return_std:
            return avg_rhos, std_rhos, avg_k, avg_kD
        ...
        std_rhos = np.nanstd(cut_rhos_array, axis=0)
