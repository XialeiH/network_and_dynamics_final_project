...
def build_torch_higher_order_complex(complex_data, device: str = "cuda") -> TorchHigherOrderComplex:
    torch = _require_torch()

    node_neighbors_dict = complex_data["node_neighbors_dict"]
    triangles_list = complex_data["triangles_list"]
    tetrahedra_list = complex_data.get("tetrahedra_list", [])
    ...
    if tetrahedra_list:
        tetrahedra = torch.tensor(
            [[index[a], index[b], index[c], index[d]] for a, b, c, d in tetrahedra_list],
            dtype=torch.long,
            device=device,
        )
    else:
        tetrahedra = torch.empty((0, 4), dtype=torch.long, device=device)

    return TorchHigherOrderComplex(
        adjacency=adjacency,
        triangles=triangles,
...
def _sample_initial_masks(n_sims: int, n_nodes: int, seed_pct: float, base_seed: int, device: str):
    torch = _require_torch()
...
def _sample_initial_masks_from_rho0s(rho0_list, n_nodes: int, base_seed: int, device: str):
    torch = _require_torch()
    masks = torch.zeros((len(rho0_list), n_nodes), dtype=torch.bool, device=device)
    gen = torch.Generator(device=device)
    for idx, rho0 in enumerate(rho0_list):
        n_initial = min(max(int(round(float(rho0) * n_nodes)), 0), n_nodes)
        if n_initial <= 0:
            continue
        gen.manual_seed(base_seed + idx)
        order = torch.randperm(n_nodes, generator=gen, device=device)
        masks[idx, order[:n_initial]] = True
    return masks


def _simplex_support_counts(infected: "torch.Tensor", simplices: "torch.Tensor", active_if_infected_count: int, n_nodes: int):
    torch = _require_torch()
    batch = infected.shape[0]
    simplex_size = simplices.shape[1] if simplices.ndim == 2 else 0
    if simplices.numel() == 0 or simplex_size == 0:
        return torch.zeros((batch, n_nodes), dtype=torch.int32, device=infected.device)

    simplex_state = infected[:, simplices]
    infected_counts = simplex_state.sum(dim=-1)
    active = infected_counts == active_if_infected_count
    susceptible = (~simplex_state) & active.unsqueeze(-1)
    support = torch.zeros((batch, n_nodes), dtype=torch.int32, device=infected.device)
    for pos in range(simplex_size):
        node_ids = simplices[:, pos].unsqueeze(0).expand(batch, -1)
...
def run_prevalence_scan_torch_higher_order(
    complex_data,
    lambda1s,
    lambda2_target,
    lambda3_target=0.0,
    ...
    torch_complex = build_torch_higher_order_complex(complex_data, device=device)
    beta1s = (
    ...
    beta3 = mu / torch_complex.avg_k3 * lambda3_target if torch_complex.avg_k3 > 0 else 0.0
    ...
    for lambda_idx, beta1 in enumerate(beta1s):
        ...
        for _ in range(t_max + 1):
            ...
            tri_support = _simplex_support_counts(
                infected,
                torch_complex.triangles,
                active_if_infected_count=2,
                n_nodes=torch_complex.n_nodes,
            )
            p_tri = 1.0 - torch.pow(1.0 - beta2, tri_support.float())

            tet_support = _simplex_support_counts(
                infected,
                torch_complex.tetrahedra,
                active_if_infected_count=3,
                n_nodes=torch_complex.n_nodes,
            )
            p_tet = 1.0 - torch.pow(1.0 - beta3, tet_support.float())

            p_infect = 1.0 - (1.0 - p_pair) * (1.0 - p_tri) * (1.0 - p_tet)
...
def run_time_evolution_scan_torch_higher_order(
    complex_data,
    lambda_target,
    lambda2_target,
    lambda3_target,
    ...
    torch_complex = build_torch_higher_order_complex(complex_data, device=device)
    beta1 = mu / torch_complex.avg_k1 * lambda_target if torch_complex.avg_k1 > 0 else 0.0
    beta2 = mu / torch_complex.avg_k2 * lambda2_target if torch_complex.avg_k2 > 0 else 0.0
    beta3 = mu / torch_complex.avg_k3 * lambda3_target if torch_complex.avg_k3 > 0 else 0.0

    infected = _sample_initial_masks_from_rho0s(
        rho0_list=rho0_list,
        n_nodes=torch_complex.n_nodes,
        base_seed=base_seed,
        device=device,
    ...
    gen = torch.Generator(device=device)
    gen.manual_seed(base_seed + 100000)

    histories = [[] for _ in rho0_list]
    for _ in range(t_max + 1):
        infected_float = infected.float()
        for idx in range(len(rho0_list)):
            histories[idx].append(float(infected_float[idx].mean().item()))

        infected_neighbor_counts = infected_float @ torch_complex.adjacency
        p_pair = 1.0 - torch.pow(1.0 - float(beta1), infected_neighbor_counts)
        ...
        p_infect = 1.0 - (1.0 - p_pair) * (1.0 - p_tri) * (1.0 - p_tet)
        susceptible = ~infected
        infect_draw = torch.rand((len(rho0_list), torch_complex.n_nodes), generator=gen, device=device)
        new_infected = susceptible & (infect_draw < p_infect)

        recover_draw = torch.rand((len(rho0_list), torch_complex.n_nodes), generator=gen, device=device)
        recoverable = infected & (~new_infected)
        new_recovered = recoverable & (recover_draw < mu)
    ...
    return [
        ...
        for rho0, history in zip(rho0_list, histories)
    ]
